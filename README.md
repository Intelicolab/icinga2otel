...
The `metric_munge` option can be used to modify metric names before sending to the OpenTelemetry collector.  This is useful in cases where you want to normalize metric naming or reduce cardinality of metrics.  The specs are given as {regexp};{replacement} and a comma separated list can be given.  When a match produces a change no further processing of remaining rules are considered.  For example, CPU load returned as "load1", "load5", "load15", could be converted to system.cpu.load_1 (or 5, or 15) by adding  --metric_munge "^load(\d\*)\$;system.cpu.load_\$1".  Some service checks, for efficiency, may collect a list of metrics in one request and return dynamically generated metrics in the result. As an example consider a temperature sensors check that returns performance data for temp_sensor1, temp_sensor2...temp_sensorN.  To reduce cardinality on the OpenTelemetry backend and make this data easier to work with, you could add --metric_munge "temp_sensor\d\*;hw.temperature", which would store each metric record using the metric name "hw.temperature".  The original metric name is always stored in an attribute called "metric.orig.name" and that could be used to filter queries used in your observability system to the particular sensor you wish to report on.  Note: adding munge rules will increase the work load for processing metrics so it is advised to use these sparingly on large systems.
